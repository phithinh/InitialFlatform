...
{
    (uint8_t*)CompositeVCPMass_DeviceDescriptor,
    VIRTUAL_COM_PORT_SIZ_DEVICE_DESC
};

ONE_DESCRIPTOR Config_Descriptor =
{
    (uint8_t*)CompositeVCPMassDevice_ConfigDescriptor,
    CDCMSC_SIZ_CONFIG_DESC
};
...
void Virtual_Com_Port_Reset(void)
{
    ...
    pInformation->Current_Feature = CompositeVCPMassDevice_ConfigDescriptor[7];
    ...
    Bot_State = BOT_IDLE;
    /*End of Init for Mass Storage Device*/  
#endif
...
RESULT Virtual_Com_Port_Data_Setup(uint8_t RequestNo)
{
    ...
    if ((Type_Recipient == (CLASS_REQUEST | INTERFACE_RECIPIENT))
        && (RequestNo == GET_MAX_LUN) && (pInformation->USBwValue == 0)
            &&(pInformation->USBwIndex0 == 0x02) && (pInformation->USBwLength == 0x01))
    {
...
RESULT Virtual_Com_Port_NoData_Setup(uint8_t RequestNo)
{
    ...
    if ((Type_Recipient == (CLASS_REQUEST | INTERFACE_RECIPIENT))
        && (RequestNo == MASS_STORAGE_RESET) && (pInformation->USBwValue == 0)
            &&(pInformation->USBwIndex0 == 0x02)&& (pInformation->USBwLength == 0x00))
    {
        /* Initialize Endpoint 1 */
        ClearDTOG_TX(ENDP4);
